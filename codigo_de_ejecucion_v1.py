...
def procesar_datos(df, local_data):
    ...
    sov_transport_time = df_work['sov_transport_time'].values[0]  # Usar la columna del DataFrame
    go_window_time = df_work['go_window'].values[0]  # Usar la columna del DataFrame
    ...
    while current_date <= df_work['date'].max():
        ...
        if df_work.loc[df_work['date'] == current_date, 'sov_efec_work_time'].values[0] > sov_transport_time:
            df_work.loc[df_work['date'] == current_date, 'sov_efec_work_time'] -= sov_transport_time
            ...
            for day in range(15):
                ...
                if df_work.loc[df_work['date'] == current_date, 'sov_efec_work_time'].values[0] > go_window_time:
                    df_work.loc[df_work['date'] == current_date, 'sov_efec_work_time'] -= sov_transfer_time  # Descontamos el tiempo
                else:
                    df_work.loc[df_work['date'] == current_date, 'sov_efec_work_time'] = 0  # No puede trabajar
